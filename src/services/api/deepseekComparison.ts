...
export async function sendDeepSeekComparison(
  ...
  const baseUrl = request.config.baseUrl || DEFAULT_BASE_URLS.deepseek
  ...
  try {
    ...
    if (!content) {
      return {
        success: false,
        error: 'No response content received from DeepSeek',
      }
    }
    ...
    if (error instanceof TypeError && error.message === 'Failed to fetch') {
      return {
        success: false,
        error: 'Cannot access DeepSeek API from browser due to CORS restrictions. Consider using a local proxy or server-side implementation.',
      }
    }
